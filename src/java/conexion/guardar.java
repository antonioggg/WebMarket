...
public class guardar {
    ...
    public void guardarCategoria(String nom, String per, int a){
     
   bd.conectarBase();
   sql="INSERT INTO categoria (nombre,padre, prod) VALUES ('"+nom+"','"+per+"',"+a+")";
        try {
            bd.sentencia.execute(sql);
        ...
        JOptionPane.showMessageDialog(null,"Se ah agregado correctamente la nueva categoría");
   }

  public void guardarcategoriaproducto(String cat,int referencia){
   bd.conectarBase();
       sql="INSERT INTO categoriaproducto (idcategoria,idproducto) VALUES ('"+cat+"',"+referencia+")";
        try {
            bd.sentencia.execute(sql);
        } catch (SQLException ex) {
            Logger.getLogger(guardar.class.getName()).log(Level.SEVERE, null, ex);
  ...
  public void especificacioproducto(int referencia,String desc,String esp){
   bd.conectarBase();
       sql="INSERT INTO especificacioproducto (descripcion,especificacion,idproducto) VALUES ('"+desc+"','"+esp+"',"+referencia+")";
        try {
            bd.sentencia.execute(sql);
        } catch (SQLException ex) {
            Logger.getLogger(guardar.class.getName()).log(Level.SEVERE, null, ex);
  ...
  public void guardarImagen(int referencia,String nomb){
   bd.conectarBase();
       sql="INSERT INTO imagenes (idproducto,imagen) VALUES ("+referencia+",'"+nomb+"')";
        try {
            bd.sentencia.execute(sql);
        } catch (SQLException ex) {
            Logger.getLogger(guardar.class.getName()).log(Level.SEVERE, null, ex);
   ...
   bd.conectarBase();
       sql="INSERT INTO Productos (Nombre,Proveedor,precio,numero) VALUES ('"+p.getNombre()+"','"+p.getProvedor()+"',"+p.getPrecio()+","+p.getNumero()+")";
        try {
            bd.sentencia.execute(sql);
	...
	String date = sdf.format(new Date()); 
        ...
        for (int i=0;i<compras.size();i++) {
            ...
            String sql3 = "INSERT INTO compras (idorden, idproducto, cantidad) Values ('"+idordencompra+"', '"+com.get(i).getRefprod()+"','"+com.get(i).getCantidad()+"' )";
            try {
